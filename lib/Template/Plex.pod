...
recursive calls to C<plex>, if no options are provided
...
new line. Does not effect C<@{[]}> on single line or embedded with other text
...
Similar to the C<plex> sub, however loads and immediately execute the template.
Equivalent to:

	my $tempalte=plex ...;
	$tempalte->render;

Arguments are the same as C<plex>.

Useful for one off rendering of a template.  If 
...
Note that the lexically aliased variables setup in C<plex> are independent to the
...
executed, however, the empty array returned, it won't inject unwanted 'last
statement' values into your template.

If you want the last statement returned into the template, use the built in
C<do>.

	eg
		
		@{[
			# This will define a package variable for use later in the template
		...
		@{[
			# This will dfine a package variable for use later in the tamplate
