...
If the number of arguments passed to the cache functions/method is 1, it is
assumed to be a path and an implicit cache key is used.

If the number of arguments larger than 1, and the second argument is a hash ref
of variables, then implicit cache key is used and the first argument is
expected to be a path.

Otherwise explicit cache key is expected as the first argument and the second
arguement is expected to be a path.
