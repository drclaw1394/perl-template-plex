...
If C<$path> is filehandle, or GLOB ref, it is slurped. Can be used to use C<__DATA__> section as a template store

If C<$path> is an array ref, the items of the array are joined into a stirng, which is used directly as the template.

The C<$hash_ref> provides variables to the template. Using refaliasing it is aliased to C<%fields> in the render subrotine. The top level items are also aliased into the render subroutine.

$root Is a directory path which if present, is prepeneded to any file path used C<plex>. This also applies to 'recursive' templates. This is aliased into the rendere to a variable C<$root>
