...
aliased into the template (more on this below). It improves the style and usage
...
C<$fields-E<gt>{name}>) or by delimiting with custom syntax C<E<lt>%= name E<gt>%>
...
options. Plex leverages perl to have a more concise API.
