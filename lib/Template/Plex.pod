...
Conceptually, a Plex template is just string returned from a subroutine in
perl's double quoted context, with the outer operators removed:
...
aliased into the template (more on this below). It improves the style and use
of variables in the template while allowing sub templates to access/override
variables in a lexical fashion. 

It also allows an external system to operate on its own variables, without
knowledge of the template mechanism API.
...
=item * Lexical and package variables and sub routines with templates 
...
C<$fields-E<gt>{name}>) or by delimiting with custom syntax C<E<gt>%= name E<lt>%>
...
If C<$variables_hash> was supplied during a C<plex> call, the top
