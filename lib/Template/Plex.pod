...
=item * Advanced templating with subclasses (in v0.4)
	...
	Build your own relational templates on top of Template::Plex. Focus on
...
Also C<${\ EXPR }> is a short way to execute function and method calls in double quoted strings.

=item * 

Other templating system are very powerful, but have huge APIs and
...
The return value is C<Template::Plex::Base> (or subclass) object which can be rendered using the
C<render> method
...
This gives the ability to extent and morph the templating system into some cool
and  wild places

The current instance of template is stored in the lexically scoped C<$plex>
variable, which is directly accessible in a template:
	
		@{[
			$plex->my_complicated_method
		]}
...
This sub returns a hash ref to the options (ie the C<%options>
hash) used in the template creation

=head3 C<args>

	$plex->args
	
	Returns the hash ref of input variables (ie the C<%fields> hash) of the template.
