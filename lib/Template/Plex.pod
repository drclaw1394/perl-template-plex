...
with the outer operators removed:
...
do just about anything in the templates. After all the template is just perl. 

The 'lexical' part of this modules is due to ability of variables to be aliased into
the template (more on this below). It improves the style and use of variables
in the template. It also allows the external system to operate on its own
variables,without knowledge of the template mechanism
...
(i.e. C<$name>) instead of as a member of a hash ref (i.e. C<$fields-E<gt>{name}>) or by delimiting with custom syntax

=item * 

The perl syntax C<@{[...]}>  will execute arbitrary perl statements in a double
quoted string. That's the hardest part of a template system and perl does it
for us

=item * 

Other templating system are very powerful, but have huge a huge APIs. Plex
leverages perl to have a more concise API.
...
This is a required argument but can be an empty hash ref C<{}> or C<undef>.

The C<$variables_hash> provides variables to the template. It is aliased to
C<%fields> variable (note the %) and directly usable in the template like a
normal hash e.g. C<$fields{name}>
...
If the C<$variables_hash> is and empty hash ref C<{}> or C<undef> then no
...
C<my> variables will only be visible within the C<@{[..]}> block it was defined.
	...
	eg	#Recursively load and render a template with my variables
		@{[ do {
			my $hash={carrots=>20};
			my $t=plex "sub/template.plex" ,$hash;
			$t->render;
		}]}
...
Sub templates loaded with C<include> will have direct access to the package
variables. However, recursive sub templates will need to have the variables
passed in the call to C<new> or C<plex>
...
=item  * Explicit inclusion of top level variables into sub templates 

For sub templates which use the C<include> mechanism, they become part of the
top level template. As such, they have access to the lexically aliased
variables and package variables automatically.

However, if you recursively use C<new> or C<plex>, you would need to manually
pass any variables you need to the sub template. Changes to the original
aliased variables inside and outside the template will still propagate into the
sub template.
...
not values), then the aliasing setup during a C<plex> or C<new> call will not
reflect what you want.

However the C<render> method call allows a hash ref containing values to use.
The hash is aliased to the C<%fields> variable in the template.
...
Please report any bugs and feature requests on the repo page: L<GitHub|http://github.com/drclaw1394/perl-template-plex>
