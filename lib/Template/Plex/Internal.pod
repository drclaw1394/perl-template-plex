...
processing template language and system to load, cache, initialize, inherit,
fill and render templates.
...
=head1 INHERITANCE MODEL

Templates can be reused by way of inheritance and slots. A slot is named
location which a child template can inject/override content in a  parent
template

Multiple slots can be declared in a parent template
...
L<Template::Plex::Base> and use the C<render> API from that class.
...
In addition to loading a template, the C<render> method are
called on it. 
...
Specified the template which will become the current templates  parent. The current template will be rendered into the default stop of the parent.

=head2 slot

	@{[slot name=>$value]}

Declares a slot in a template which can be filled by a child template calling C<fill_slot> directives.
...
The default slot can not be specified. It is filled with the rendered result of the child template.
